...
import Discovery from '#src/torrent/discovery';
import exchangeMetadata from '#src/torrent/exchange-metadata';
...
const thruthy = () => true;
const falsy = () => false;
const toNumber = (val) => { if (val === true) return 1; return val || 0; };
...
export default class Torrent extends EventEmitter {
  constructor(source, options = {}) {
    ...
    this.debug('new %O', source, options);
    ...
    this.rechokeSlots = (options.uploads === false || options.uploads === 0)
      ? 0 : (+options.uploads || 10);
    ...
    const onMetadata = (parse) => {
      ...
      const onInterested = () => { this.swarm.resume(); };
      const onUninterested = () => { this.swarm.pause(); };
      ...
      const gc = () => {
        for (let i = 0; i < this.selection.length; i += 1) {
          ...
          while (!this.pieces[s.from + s.offset] && s.from + s.offset < s.to) s.offset += 1;
      ...
      const onUpdateTick = () => { process.nextTick(onUpdate); };
      const onPieceComplete = (index, buffer) => {
        if (!this.pieces[index]) return;

        this.store.put(index, buffer, (err) => {
          if (err) { onUpdateTick(); return; }

          this.pieces[index] = null;
          this.reservations[index] = null;
          this.bitfield.set(index, true);

          for (let i = 0; i < this.wires.length; i += 1) this.wires[i].have(index);
      ...
      const onHotswap = this.hotswap === false ? falsy : (wire, index) => {
        const speed = wire.downloadSpeed();
        if (speed < Piece.BLOCK_LENGTH) return false;
        if (!this.reservations[index] || !this.pieces[index]) return false;
        ...
        for (let i = 0; i < r.length; i += 1) {
          ...
          if (other && other !== wire) {
            ...
            if (
              ...
              && !(2 * otherSpeed > speed || otherSpeed > minSpeed)
            ) {
              min = other;
              minSpeed = otherSpeed;
            }
        ...
        for (let i = 0; i < min.requests.length; i += 1) {
          ...
          if (req.piece === index) {
            this.pieces[index].cancel(parseInt(req.offset / Piece.BLOCK_SIZE, 10));
          }
        }
      ...
      const onRequest = (wire, index, hotswap) => {
        ...
        wire.request(index, offset, size, (err, block) => {
          ...
          if (p !== this.pieces[index]) { onUpdateTick(); return; }
          ...
          if (!p.set(reservation, block, wire)) { onUpdateTick(); return; }
          ...
          if (sha1(buffer) !== parse.pieces[index]) {
            ...
            sources.forEach((wireSource) => {
              ...
              const badPieceStrikes = wireSource.badPieceStrikes.filter(
                (strike) => (now - strike) < BAD_PIECE_STRIKES_DURATION,
              );
              Object.assign(wireSource, { badPieceStrikes });
              wireSource.badPieceStrikes.push(now);

              if (wireSource.badPieceStrikes.length > BAD_PIECE_STRIKES_MAX) {
      ...
      const onValidateWire = (wire) => {
        ...
        for (let i = this.selection.length - 1; i >= 0; i -= 1) {
          ...
          for (let j = next.to; j >= next.from + next.offset; j -= 1) {
      ...
      const shufflePriority = (i) => {
        let last = i;
        for (let j = i; j < this.selection.length && this.selection[j].priority; j += 1) {
          last = j;
        }
      ...
      const selectWire = (wire, hotswap) => {
        ...
        for (let i = 0; i < this.selection.length; i += 1) {
          const next = this.selection[i];
          for (let j = next.from + next.offset; j <= next.to; j += 1) {
      ...
      onUpdate = () => { this.wires.forEach(onUpdateWire); };
      const onWire = (wire) => {
        ...
        const onChokeTimeout = () => {
          if (
            this.swarm.queued > 2 * (this.swarm.size - this.swarm.wires.length)
            && wire.amInterested
          ) {
            wire.destroy();
            return;
        ...
        wire.on('close', () => { clearTimeout(id); });
        wire.on('choke', () => {
          clearTimeout(id);
          id = setTimeout(onChokeTimeout, timeout);
        });
        wire.on('unchoke', () => { clearTimeout(id); });
      ...
      const onRechoke = () => {
        if (this.rechokeOptimisticTime > 0) this.rechokeOptimisticTime -= 1;
        ...
        for (; i < peers.length && unchokeInterested < this.rechokeSlots; i += 1) {
          peers[i].isChoked = false;
          if (peers[i].interested) unchokeInterested += 1;
        }

        if (!this.rechokeOptimistic && i < peers.length && this.rechokeSlots) {
          const candidates = peers.slice(i).filter((peer) => peer.interested);
          const optimistic = candidates[parseInt(Math.random() * candidates.length, 10)];
      ...
      const onRunning = () => {
        ...
        this.rechokeIntervalId = setInterval(onRechoke, RECHOKE_INTERVAL);
      ...
      this.pieces = parse.pieces.map(
        (hash, i) => new Piece(i !== parse.pieces.length - 1 ? pieceLength : lastPieceLength),
      );
      ...
      this.files = parse.files.map((fileMetadata) => {
        ...
        const endPiece = parseInt((file.offset + file.length - 1) / pieceLength, 10);

        file.select = () => { selectFile(offsetPiece, endPiece, false); };
        file.deselect = () => { deselectFile(offsetPiece, endPiece, false); };
      ...
      const validPiece = (index) => {
        this.pieces[index] = null;
        this.bitfield.set(index, true);
        this.debug('piece %s valid', index);
        this.emit('verify', index);
      };
      const verify = () => {
        if (this.complete && !this.verified) {
          ...
          pAll(this.pieces.map(
            (piece, index) => () => {
              if (this.destroyed) return new Error('torrent is destroyed');
              const invalidPiece = (i) => { this.debug('piece %d invalid', i); };
              return pify(this.store.get.bind(this.store))(index)
                .then((buf) => {
                  if (sha1(buf) !== parse.pieces[index] || !this.pieces[index]) {
                    invalidPiece(index);
                  } else {
                    validPiece(index);
                  }
            ...
            .then(() => { this.verified = true; onReady(); })
            .catch(noop);
        }
      };
    ...
    const getaMetdata = () => {
      ...
      const startDiscovery = () => {
        ...
        this.swarm.on('wire', (wire, connection) => {
          Object.assign(wire, { get origin() { return `${connection.remoteAddress}:${connection.remotePort}`; } });
  ...
  get downloaded() {
    ...
    return this.pieces.reduce((downloaded, piece, index) => {
      if (this.bitfield.get(index)) {
        ...
        return downloaded + ((index !== this.pieces.length - 1) ? pieceLength : lastPieceLength);
      }
  ...
  get timeRemaining() {
    ...
    return parseInt(((this.parse.length - this.downloaded) / this.downloadSpeed) * 1000, 10);
  }
