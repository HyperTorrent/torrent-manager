...
const sha1 = (data) => crypto.createHash('sha1')
  .update(data)
  .digest('hex');

export default (infoHash, metadata, callback) => {
  let metadataPieces = [];
    ...
    wire.once('extended', (id, encodedHandshake) => {
      ...
      wire.on('extended', (extId, ext) => {
        ...
        if (message.msg_type === 0) {
          ...
          const buf = wireMetadata.slice(offset, offset + METADATA_BLOCK_SIZE);
          wire.extended(
            channel,
            Buffer.concat([bncode.encode({ msg_type: 1, piece }), buf]),
          );
          return;
        }
